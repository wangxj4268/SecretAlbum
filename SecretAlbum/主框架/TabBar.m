...
- (void)layoutSubviews {
    ...
    CGFloat w = kSelfWidth/5.0;
    
    UIButton *publishBtn = [[UIButton alloc] init];
    [publishBtn setImage:[UIImage imageNamed:@"car_easy_tabbar_realtime_press"] forState:UIControlStateNormal];
    ...
    publishBtn.centerX = kSelfWidth / 2;
    ...
    for (NSUInteger i = 0 , j = 0; i < count; i++) {
        ...
        if ([view isKindOfClass:class]) {
            view.width = kSelfWidth / 5.0;
            view.x = kSelfWidth * j / 5.0;
            j++;
            if (j == 2) {
                j++;
            }
